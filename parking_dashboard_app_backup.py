...
class ParkingAnalyzer:
    ...
    def process_videos_with_annotations(self, vehicle_video_path, parking_video_path):
        ...
        # Process vehicle video
        vehicle_output_path = output_dir / f"annotated_vehicle_{int(time.time())}.mp4"
        self.create_annotated_vehicle_video(vehicle_video_path, str(vehicle_output_path))
        
        # Process parking video
        parking_output_path = output_dir / f"annotated_parking_{int(time.time())}.mp4"
        self.create_annotated_parking_video(parking_video_path, str(parking_output_path))
        
        return str(vehicle_output_path), str(parking_output_path)
    
    def create_annotated_vehicle_video(self, input_path, output_path):
        """Create annotated vehicle detection video"""
        ...
        while True:
            ...
            cv2.putText(frame, f"Currently Visible: {len(results[0].boxes) if results[0].boxes is not None else 0}", 
    ...
    def create_annotated_parking_video(self, input_path, output_path, grid_size=(6, 4)):
        """Create annotated parking space video"""
